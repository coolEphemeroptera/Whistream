...
class WhisperModel:
    ...
    def read_data(self):
        while self.running:
            if self.Q_audio_asr.qsize()>=self.read_size:
                with self.lock:
                    for _ in range(self.read_size):
                        self.buffer["start"] += 1
                        self.buffer["end"] += 1
                        self.buffer["data"].append(self.Q_audio_asr.get())
            time.sleep(0.1)
        self.P("Stop to read data ..")
    ...
    def transcribe(
        ...
        segments = self.generate_segments(tokenizer, options, encoder_output)
    ...
    def generate_segments(
        ...
        latest = 0
        while self.running:
            self.P(f"ASR ===> the seek:{format_timestamp(seek*seconds_per_frame)}, the latest:{format_timestamp(self.buffer['end']/sr)}")
            
            with self.lock:
                latest = self.buffer["end"]

            if (seek+segment_size)*seconds_per_frame*sr<=latest:
                stime = time.time()
                ...
                # 提取chunk data
                self.P("extract acoutic features ..")
                ws = int(seek*seconds_per_frame*sr)
                we = int((seek+segment_size)*seconds_per_frame*sr)
                with self.lock:
                    wav_data = list(self.buffer["data"])[ws-self.buffer["start"]:we-self.buffer["start"]]
                wav_data = np.float32(np.array(wav_data)/(2**(width-1))) 
                segment = self.feature_extractor(wav_data, chunk_length=30)
                self.P("extract 30s chunk data ..")
                ...
                self.P(f"no_speech_prob:{result.no_speech_prob}")
                self.P(f"avg_logprob:{avg_logprob}")
                ...
                for segment in current_segments:
                    ...
                    self.asr_results.append([tmp_res.start,tmp_res.end,tmp_res.text])
                    self.P(f"%s -> %s %s"%(format_timestamp(tmp_res.start),format_timestamp(tmp_res.end),tmp_res.text))
                ...
                self.P(f"ASR ====> cost {time.time()-stime} seconds")
                
            time.sleep(1)
        self.P("Stop to recognition ..")
    ...
    def init_state(self,Q_audio_asr:Queue,read_size=sr):
        self.running = True
        self.buffer = {"start":0,
                        "end":0,
                        "data":deque([-1]*10*60*sr)}
        self.read_size = read_size
        self.lock = threading.Lock()
        self.Q_audio_asr = Q_audio_asr
        self.asr_results = []
